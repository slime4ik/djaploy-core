...
| Подключение | `ssh.go` → `DialPassword`, `hostkey.go` | Одно SSH-соединение, пароль или keyboard-interactive. Ключ хоста сверяется с отпечатком, записанным на первом деплое: расхождение рвёт соединение до того, как уйдёт пароль. Если вход не под root, проверяем `sudo` и поднимаем права только там, где надо (`deployer.go` → `priv`). |
| Ключ доступа | `deployer.go` → `installAccessKey` | Генерим ed25519-ключ, публичную половину дописываем в `authorized_keys`, приватную шифруем и сохраняем. Ради этого последующие деплои идут без пароля. |
...
| `~/.ssh/authorized_keys` | первый деплой | Дописываем наш публичный ключ. Полное удаление эту строку убирает. |
...
- **Проверка ключа хоста работает по принципу trust on first use.** Первое подключение запоминает
  ...
  как уйдёт пароль.
- **Git-токен и пароль sudo видны в аргументах процесса на твоём же сервере.** Токен уходит в
  аргумент `git -c http.extraheader=...`, пароль в `printf ... | sudo -S`. В наших логах и в URL их
  нет, но тот, кто в этот момент может выполнить `ps` на сервере, их прочитает. На личном VPS это
  никто, на общей машине это настоящая утечка.
